...
Output: AttendanceDashboard.html  — single self-contained file, upload to Google Drive or anywhere

Steps:  1. Drop updated xlsx in the same folder
        2. Run:  python3 build.py
        3. Share AttendanceDashboard.html
"""
...
TEMPLATE     = os.path.join(SCRIPT_DIR, 'index.html')
OUTPUT_FILE  = os.path.join(SCRIPT_DIR, 'AttendanceDashboard.html')  # for Google Drive
DATA_JS      = os.path.join(SCRIPT_DIR, 'data.js')                    # for GitHub Pages
...
if '<script src="data.js"></script>' not in html:
    print("ERROR: Could not find <script src=\"data.js\"></script> in index.html")
    print("Make sure index.html is in the same folder as build.py")
    sys.exit(1)

merged = html.replace('<script src="data.js"></script>', '<script>\n' + data_js + '\n</script>')
